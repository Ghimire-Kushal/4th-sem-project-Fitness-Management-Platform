...
exports.createPlan = async (req, res) => {
  try {
    const plan = await DietPlan.create({ ...req.body, trainer: req.user._id });
    await Notification.create({
      user: plan.member,
      title: "New Diet Plan",
      message: `Your trainer has assigned you a new diet plan: "${plan.title}".`,
      type: "plan",
    });
...
exports.getMyPlans = async (req, res) => {
  try {
    const plans = await DietPlan.find({ member: req.user._id, isActive: true })
